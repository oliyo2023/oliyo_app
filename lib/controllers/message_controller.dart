...
class MessageController extends GetxController {
  ...
  Future<void> loadMessages({bool refresh = false}) async {
    ...
    try {
      ...
      // 查询消息 - 包括发送和接收的消息
      final resultList = await pbService.pbClient.collection('messages').getList(
        page: page,
        perPage: perPage,
      ...
      final newMessages = resultList.items.map((item) => Message.fromJson(item.toJson())).toList();
  ...
  Future<void> sendMessage(String receiverId) async {
    ...
    try {
      ...
      final record = await pbService.pbClient.collection('messages').create(body: {
        'content': content,
  ...
  Future<void> markAsRead(String messageId) async {
    try {
      await pbService.pbClient.collection('messages').update(messageId, body: {
        'is_read': true,
      });
